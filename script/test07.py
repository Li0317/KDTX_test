...
class TestAddCourse:
    ...
    def test01_select_success(self):
        response = self.course_api.select_course(test_data="?name=English", token=TestAddCourse.token)
        print(response.json())
    ...
    def test02_select_fail(self):
        response = self.course_api.select_course(test_data="?name=English", token="xxx")
        print(response.json())
        assert 200 == response.status_code
